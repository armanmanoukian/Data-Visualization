...
ggplot(data = merged_data, aes(x = PM2.5_AQI_Value, y = Annual_Lung_Cancer_Deaths, color = Country)) +
  geom_point(aes(size = ifelse(Country == 'China', 6, 3))) +
  geom_text(aes(label = ifelse(Country %in% c('China', 'Japan', 'India'), Country, '')), 
            vjust = -0.5, hjust = 0.5, size = 3, fontface = "bold", color = 'black') +
...
annotate("text", x = Inf, y = -Inf, label = "Annual_Lung_Cancer_Deaths", 
