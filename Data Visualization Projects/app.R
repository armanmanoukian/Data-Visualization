  ...
  sidebarLayout(
    ...
    mainPanel(
      tabsetPanel(
        ...
        tabPanel("📍 Fiction Rate Map", plotOutput("fictionMapPlot")),
...
server <- function(input, output) {
  ...
  output$fictionMapPlot <- renderPlot({
    df_loc_fiction <- df %>%
      separate_rows(genres, sep = ",\\s*") %>%
      mutate(FictionReader = ifelse(genres == "Fiction", 1, 0)) %>%
      group_by(Location) %>%
      summarise(FictionRate = mean(FictionReader), .groups = "drop") %>%
      mutate(Country = str_to_title(Location), CountryCode = countrycode(Country, origin = "country.name", destination = "iso3c")) %>%
      filter(!is.na(CountryCode))
    
    world <- map_data("world")
    world_map <- left_join(world, df_loc_fiction, by = c("region" = "Country"))
    
    ggplot(world_map, aes(x = long, y = lat, group = group, fill = FictionRate)) +
      geom_polygon(color = "white") +
      scale_fill_viridis_c(option = "inferno", na.value = "grey90") +
      labs(title = "Fiction Reading Rate by Country") +
      theme_void()
  })
