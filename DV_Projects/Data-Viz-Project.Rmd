...
```{r Age Distribution of Readers1}
ggplot(df_clean, aes(x = Age)) +
  geom_histogram(binwidth = 5, fill = "steelblue", color = "black") +
  labs(title = "Age Distribution of Readers", x = "Age", y = "Count")
```
...
ggplot(df_genres[1:15,], aes(x = reorder(genres, count), y = count)) +
  geom_bar(stat = "identity", fill = "#FF8C00", width = 0.7) +
  geom_text(aes(label = count), hjust = -0.1, size = 3.8, color = "black") +
  coord_flip() +
  labs(
    title = "🎭 Top 15 Most Common Genres Among Readers",
    subtitle = "Based on cleaned genre data; genres separated by commas",
    x = "Genre",
  ...
  theme_minimal(base_size = 13) +
  theme(
    plot.title = element_text(face = "bold", size = 16, color = "#E67E22"),
    plot.subtitle = element_text(size = 12, color = "#5D4037"),
    plot.caption = element_text(size = 10, face = "italic", color = "gray40"),
    axis.title = element_text(face = "bold"),
    panel.grid.major.y = element_blank(),  # cleaner
    panel.grid.minor = element_blank()
  ) +
  expand_limits(y = max(df_genres$count[1:15]) * 1.1)  # space for text labels
```
